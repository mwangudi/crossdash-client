...
import WitnessPreviewCard from './WitnessPreviewCard';
...
const Form = props => {
  ...
  return (
    <Wrapper className="p-3 w-100 mt-2 mb-3" elevation={3}>
      <h6 className="formTitle">Witness Details</h6>
      <hr />
      <form id={id} onSubmit={handleSubmit}>
        <div className="row">
          <div className="col-lg-6 col-12">
            ...
            <Switch
              id="isAdult"
              label="Was the witness an adult?"
            ...
            <div className="row p-3 content-preview">
              {React.Children.toArray(
                witnesses.map(witness => (
                  <WitnessPreviewCard witness={witness} />
                )),
              )}
            </div>
          </div>
...
const FormikEnhancer = withFormik({
  ...
  validationSchema: Yup.object().shape({
    ...
    gender: Yup.string()
      .required()
      .label('Gender'),
    isAdult: Yup.bool().label('Witness is an adult'),
    address: Yup.string().label('Witness physical address'),
  }),
